...
Shader "Valkyrie/ImageEffect/Unlit/MobileBloom" {
	Properties {
	...
	CGINCLUDE
		...
		#define BLOOM_INTENSITY _Parameter.z
		...
		{				
			...
			return saturate(color - ONE_MINUS_INTENSITY) * BLOOM_INTENSITY;
		}
		
		fixed4 fragDownsample ( v2f_withMaxCoords i ) : SV_Target
		{				
			fixed4 color = tex2D(_MainTex, i.uv2[4]);
			color += tex2D (_MainTex, i.uv2[0]);
			color += tex2D (_MainTex, i.uv2[1]);
			color += tex2D (_MainTex, i.uv2[2]);
			color += tex2D (_MainTex, i.uv2[3]);
			return max(color/5 - ONE_MINUS_INTENSITY, 0) * BLOOM_INTENSITY;
		} 
		...
		half4 fragBlurSGX ( v2f_withBlurCoordsSGX i ) : COLOR
		{
			...
			half4 color = tex2D(_MainTex, i.offs[6]) * curve[3];
		...
		{	
			...
			fixed4 screencolor = tex2D(_Bloom, i.uv.zw)*_Parameter.z*_ColorMix;
			//color += screencolor;
			//return color;
			return 1 - (1 - color)*(1 - screencolor);
		}	
